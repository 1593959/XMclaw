...
| | |
|---|---|
| **Apprentice-style self-improvement (Epic #24)** | Six always-on observers turn each turn into evolution signal: **HonestGrader** scores tool results on hard evidence (ran/returned/type-matched/side-effect-observable, summed at 0.80; LLM self-rating capped at 0.20). **JournalWriter** writes one row per session to `~/.xmclaw/v2/journal/<YYYY-MM>/` — turn count, tool calls, grader stats. **ExtractFactsHook** (B-319) runs at every turn end and asks the LLM for facts in 6 buckets — workflow / tool_quirks / failure_modes / values / rules / preferences — routed in a single round-trip to AGENTS.md / TOOLS.md / MEMORY.md / SOUL.md / LEARNING.md / USER.md respectively (`USER_PROFILE_UPDATED` event invalidates the system-prompt cache so the next turn picks them up). **SkillDreamCycle** runs every 30 min and **RealtimeEvolutionTrigger** runs after each turn (debounced): both walk recent journal patterns and emit `SKILL_DRAFTED` events that **ProposalMaterializer** turns into runnable v1 skills under `~/.xmclaw/skills_user/`. **EvolutionEvaluationTrigger** (B-294) debounces grader verdicts and fires `EvolutionAgent.evaluate()` when enough new signal lands; the controller checks the 4-gate threshold and emits `SKILL_PROMOTION_RECOMMENDED` (B-318). **MutationOrchestrator** synthesises `v(N+1)` for skills whose EWMA drops, via DSPy/GEPA. The orchestrator defaults to `auto_apply=False` — humans approve via `xmclaw evolve review` / `xmclaw evolve approve <id>`. Every promotion goes through evidence-gated `SkillRegistry.promote(evidence=…)` (anti-req #12 enforced at the registry door). Two tools — `recall_user_preferences` + `journal_recall` — let the agent actively pull what it learned about you from past sessions before deciding the next move. |

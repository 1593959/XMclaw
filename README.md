...
| | |
|---|---|
| **Apprentice-style self-improvement (Epic #24)** | Five always-on observers turn each turn into evolution signal: **HonestGrader** scores tool results on hard evidence (ran/returned/type-matched/side-effect-observable, summed at 0.80; LLM self-rating capped at 0.20). **JournalWriter** writes one row per session to `~/.xmclaw/v2/journal/<YYYY-MM>/` — turn count, tool calls, grader stats. **ProfileExtractor** asks the LLM every 3 turns to surface durable preferences and atomically appends them to `<persona>/USER.md` (`USER_PROFILE_UPDATED` event invalidates the system-prompt cache so the next turn picks them up). **SkillDreamCycle** runs every 30 min: walks recent journal patterns and drafts new `SKILL.md` candidates as `SKILL_CANDIDATE_PROPOSED` events. **EvolutionAgent observer** aggregates grader verdicts per `(skill_id, version)` and proposes promotions when the 4-gate threshold clears. The controller defaults to `auto_apply=False` — humans approve via `xmclaw evolve review` / `xmclaw evolve approve <id>`. Every promotion goes through evidence-gated `SkillRegistry.promote(evidence=…)` (anti-req #12 enforced at the registry door). Two new tools — `recall_user_preferences` + `journal_recall` — let the agent actively pull what it learned about you from past sessions before deciding the next move. |

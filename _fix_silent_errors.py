"""Script to add ErrorAggregator.record() calls to key except blocks."""
import re
from pathlib import Path

# Severity mapping based on context heuristics
...
def get_function_name(lines_before: list[str]) -> str:
    """Walk backwards to find the current function/method name."""
...
def process_file(path: Path) -> None:
    ...
    while i < len(lines):
        ...
        if m:
            indent = m.group(1)
            # Look ahead for the pass or the end of the block
            j = i + 1
            block_lines = []
            has_pass = False
            ...
            while j < len(lines):
                blk = lines[j]
                # Stop if we hit a non-indented line (or same/less indent than the except)
                if blk.strip() and not blk.startswith(indent + " ") and not blk.startswith(indent + "\t"):
                    # Same indent or less → new block
                ...
                if blk.strip() == "pass":
                    ...
                    j += 1
                    break
                if blk.strip().startswith("return ") or blk.strip().startswith("raise "):
                    j += 1
                    break
                j += 1
            
            if has_pass and not has_existing_record:
                ...
                # Check if the except line already binds the exception
                exc_var = None
                ...
                if exc_var is None:
                    # Need to add 'as _exc' to the except line
                    line = line.rstrip("\n")
                    if line.endswith(":"):
                        line = line[:-1] + " as _exc:"
                    else:
                ...
                # Insert record call before pass
                record_line = f'{indent}    get_aggregator().record(ErrorSeverity.{severity}, __name__, "{func_name}", {exc_var})\n'
                # Replace the pass line with record + pass
                new_block = []
                for bl in block_lines:
                    if bl.strip() == "pass":
                        new_block.append(record_line)
                        new_block.append(bl)
                    else:
                        new_block.append(bl)

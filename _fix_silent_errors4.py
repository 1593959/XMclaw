"""Robust script to add ErrorAggregator.record() to ALL except Exception blocks."""
import re
...
SEVERITY_RULES = [
    ...
    (r"_run_hop_loop.*except.*exc.*latency_ms.*LLM_RESPONSE", "CRITICAL"),
        ...
        if m:
            ...
            while j < len(lines):
                ...
                if not stripped or stripped.startswith("#"):
                    ...
                    j += 1
                    continue
                # Check if this line is at the same or lower indent than the except
                if not blk.startswith(indent + " ") and not blk.startswith(indent + "\t"):
                    # Same indent or lower - this is the end of the block
                    break
                block_lines.append(blk)
                if "get_aggregator().record" in blk:
                    has_record = True

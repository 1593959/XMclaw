...
- 2026-04-25: Phase 1 Chat 骨架 + 真 WS 接入 + 流式 markdown——把 `/chat` 路由从 Phase 0 的 `<Placeholder>` 换成真正可用的 Chat workspace，不再只是空骨架。**(1)** 新增 `lib/auth.js`（同源 `GET /api/v2/pair`，硬不出域：测试 `assert "http://" not in src and "https://" not in src` 锁第三方端点）+ `lib/ws.js`（`createWsClient`：URL 形状 `/agent/v2/{sid}?token=<hex>` + `wss:`/`ws:` 跟页面协议、指数退避重连 `RECONNECT_BASE_MS=500`/`RECONNECT_MAX_MS=30000` + jitter + 12 次封顶、`4401` / `4403` 关码升 `auth_failed` 不再重连——契约见 `xmclaw/daemon/pairing.py`，`socketFactory` + `maxReconnects` 暴露成测试 seam 让静态扫描套件不打真网络）+ `lib/chat_reducer.js`（纯函数：`applyEvent` / `applySessionLifecycle` / `appendOptimisticUser` 三件套 + `PHASE_1_EVENT_TYPES` 七元组 USER_MESSAGE / LLM_CHUNK / LLM_RESPONSE / TOOL_CALL_EMITTED / TOOL_INVOCATION_FINISHED / ANTI_REQ_VIOLATION / SESSION_LIFECYCLE，乐观本地回显由 daemon 镜像回来的 user_message 用 `correlation_id` 去重）+ `lib/markdown.js`（escape-HTML 先行、用 `\u0000CB${i}\u0000` 占位提取 fenced block 防 inline pass 二次处理，bold/italic/code/autolink/`rel="noopener noreferrer"` 全锁）。**(2)** 三个 molecules：`MessageBubble.js`（拿 reducer 已 escape 过的字串走 `dangerouslySetInnerHTML`——XSS-safe 因为转义在前，渲染在后；ToolCard 子组件画 tool_invocation_finished 卡）、`MessageList.js`（用稳定 `message.id` 当 Preact key，token-keyed diff 让流式 chunk 原地补丁不重挂；`data-xmc-pinned` 实现"用户没向上滚就跟到底"的滚动行为）、`Composer.js`（Enter 发送 / Shift+Enter 换行 / Esc blur，`evt.isComposing` 守 IME 上屏不抢键）+ `pages/Chat.js` 装配。**(3)** `app.js` 整体重写连线：`fetchPairingToken` → 选 / 造 sid（localStorage 持久化 `xmc.active_sid` + `xmc.sids`）→ `createWsClient` → 每个 onEvent 派发到 reducer，连接状态进 `connection` slice 让 StatusBar 可视化 `connecting`/`connected`/`reconnecting`/`auth_failed`/`disconnected` 五态；`store.js` 加 `auth` + `chat` + `connection` slice + `persistActiveSid` / `persistSidList` / `newSid` 三个辅助；`styles/chat.css` 全部用 tokens.css 设计变量、与 `layout.css`/`atoms.css` 之间按 `index.html` link 顺序分层；`layout.css` 把 `.xmc-main` 改成 flex 列容器避免在 app.js 里写内联 style hack。**(4)** Bootstrap timer-leak 修补——Phase 1 分支从 Phase 0 拉出来时 PR-1 还没 merge，所以这里同步把 `timeoutImport` 的 setTimeout 句柄抓进 `let timer` + `Promise.race(...).finally(() => clearTimeout(timer))`，否则 CDN 慢到比 timeout 晚才到时会留 dangling 拒绝 / 二次 import。**(5)** 测试：`test_v2_ui_scaffold.py` 扩 8 个新 REQUIRED_FILES（lib/auth/ws/markdown/chat_reducer + 3 molecules + Chat page）、`chat.css` 进 stylesheet 顺序检查、`SOURCE_GLOBS` 加 `lib/*.js` + `pages/*.js` + `components/molecules/*.js`、`/ui/*` reachability 加 9 条；新增 `test_v2_ui_chat.py` 21 用例：app.js 引用 Phase-1 模块 + `/chat` 用 ChatPage + 派发 `applyEvent`/`applySessionLifecycle`/`appendOptimisticUser`、ws.js URL+`?token=`+`wss:`/`ws:`+4401/4403/`auth_failed`+`Math.pow(2,)` 退避 + `socketFactory`/`maxReconnects` 测试 seam、auth.js 仅打 `/api/v2/pair` 且无第三方 URL、chat_reducer 7 个 EventType 字符串字面量都在 + 三 helpers + `PHASE_1_EVENT_TYPES` 列表、markdown.js `escapeHtml` + `extractCodeBlocks` + `xmc-md__code` / `xmc-md__pre` 类名、4 个 molecule/page 都从 `window.__xmc` 拿 preact + htm.bind、MessageBubble 走 `dangerouslySetInnerHTML` 且导入 `renderMarkdown`、Composer 4 条 Enter / shiftKey / isComposing / Escape 键绑定都在、store.js 11 个 Phase-1 surface needle 全在；`scripts/test_lanes.yaml` `ui` lane 加 `test_v2_ui_chat.py`；`.gitignore` 加 `!xmclaw/daemon/static/lib/**` 例外，否则 Python 通用 `lib/` 规则会吞掉新目录。**(6)** 验收：ui lane 72 / 72 绿；全套 1511 passed / 7 skipped 无回归；roadmap lint 退 0；ruff 对改动文件干净。下一步：把 PR #30（Phase 0）+ Phase 1 PR 合到 main，旧 3 文件 UI（app.js / index.html / style.css）会被 StaticFiles dir-index 改读 `/ui/index.html` 后彻底替换 (commit 612ea0b)

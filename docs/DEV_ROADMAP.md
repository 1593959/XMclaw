...
- 2026-04-25: 覆盖率收紧（stacked on `b53e85a`）——`bootstrap.js` 修真 P0 timer-leak race + `/ui/` 裸路径 reachability 补测。(1) **race 修复**：旧版 `timeoutImport(url, ms)` 用 `Promise.race([import(url), new Promise((_, rej) => setTimeout(rej, ms))])`，CDN 慢回包时 `setTimeout` 即便 vendor fallback 已经赢了仍会触发——之后 CDN 真到货又跑一遍 `import(url)`，导致 `app.js` 双载、event handler 重复绑定。改为 `let timer; const timeout = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error(...)), ms); }); return Promise.race([import(url), timeout]).finally(() => clearTimeout(timer));`，无论哪边赢都清掉 timer 句柄，杜绝 dangling handle。(2) **静态扫描门卫**：`tests/unit/test_v2_ui_scaffold.py::test_bootstrap_clears_timeout_to_avoid_dangling_handles` 直接断言源码里有 `clearTimeout(` 和 `.finally(` 字面——任何后续重构想去掉清理逻辑都会被这条测拦下。(3) **`/ui/` 裸路径**：之前 ui scaffold 测只 hit `/ui/index.html` / `/ui/app.js` 这些显式资产，从未测 `StaticFiles(html=True)` 的 dir-index 行为；新加 `test_ui_bare_path_serves_index_html` 通过 TestClient `GET /ui/` 验证 200 + 返回的是 index.html 内容（regex 匹 `<title>XMclaw</title>` + `id="app"`），关上 mount 配置漂移的盲点（commit 829eea0）

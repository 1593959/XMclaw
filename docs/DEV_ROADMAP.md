...
- [x] `providers/channel/dingtalk/` + mock 测试（B-383, 2026-05-09）
- [x] `providers/channel/email/` + mock 测试（B-393, 2026-05-09 — IMAP poll + SMTP send, stdlib only）
- [x] `providers/channel/wecom/` + mock 测试（B-384, 2026-05-09 — outbound webhook only）
...
- 2026-05-09: B-380 Telegram channel adapter 真实现 — 替换 19 行 scaffold；python-telegram-bot 长轮询 + allowlist + injection scan + 4096-char chunking + lazy-import；30 条单元测试 (`tests/unit/test_v2_channels_telegram.py`)，pyproject.toml 加 `channels-telegram` 细粒度 extra (commit 9566d16)
- 2026-05-09: B-381 Discord channel adapter 真实现 — 新建 `providers/channel/discord/` 包（之前不存在）；discord.py>=2 gateway 模式 + intents (message_content + messages + guilds) + B-337 user/channel allowlist + Epic #14 injection scan + 2000-char chunking + lazy-import；35 条单元测试 (`tests/unit/test_v2_channels_discord.py`)；pyproject.toml 加 `channels-discord` 细粒度 extra (commit 0db7030)
- 2026-05-09: B-384 WeCom (企业微信) channel adapter 真实现 — 替换 19 行 scaffold；outbound-only via internal-bot webhook (`https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=...`)，pure REST POST via httpx (无新增依赖)；inbound (self-built app callback) 文档明确标记 out-of-scope (需要公网 URL + AES-CBC + 签名验证 — 等 cloudflared 通用 bootstrap 落地)；msgtype 配置（text/markdown 默认 markdown）+ mentioned_list / mentioned_mobile_list (text msgtype) + 4096-char chunking + 5xx/网络错误 retry-once + WeCom errcode 透传；`scaffold` → `ready` 迁移 (test_v2_channel_scaffold.py / test_v2_channel_phase5.py 同步移除 wecom)；39 条单元测试 (`tests/unit/test_v2_channels_wecom.py`) (commit aeb8040, source split f29ca96)
- 2026-05-09: B-383 DingTalk (钉钉) channel adapter 真实现 — 替换 7 行 scaffold；dingtalk-stream 0.20+ Stream Mode（长连 WebSocket，无需公网 IP / cloudflared）+ `AsyncChatbotHandler` 桥到 adapter event loop + B-337 sender_staff_id / conversation_id allowlist + Epic #14 injection scan + 4500-char chunking + session_webhook 缓存（最近一条 inbound 的 webhook 是 SDK 推荐的 reply 路径，无需 OpenAPI access token）+ LRU dedup（at-least-once 重连）+ lazy-import（`pip install xmclaw[channels-dingtalk]`）+ 容错（`reply_text` 返回 None 时 surface 成 RuntimeError 不静默丢）；40 条单元测试 (`tests/unit/test_v2_channels_dingtalk.py`)，`__init__.py` MANIFEST `implementation_status="ready"`，`tests/unit/test_v2_channel_scaffold.py` 把 dingtalk 从 _SCAFFOLD_CHANNELS 移走，`tests/unit/test_v2_channels_router.py` 更新 `test_list_carries_implementation_status` 断言 dingtalk=ready / weixin=scaffold；`daemon/config.example.json` 加 dingtalk 配置块；`scripts/test_lanes.yaml` 加 channels lane (commit 247b2fc)
- 2026-05-09: B-389 (Sprint 2) Composio bridge — 7000+ pre-integrated SaaS tools 接入 ToolProvider (commit f0dd0b2)。新增 `xmclaw/providers/tool/composio.py` `ComposioToolProvider(ToolProvider)`：lazy `import composio` 在 `_get_toolset()` 里（缺包不炸 daemon boot）+ `list_tools()` 把 Composio 的 OpenAI-shape function spec 转成 `ToolSpec`（接受 wrapped + flat 两种 SDK 形状，单条坏 entry 静默跳过）+ `invoke()` 走 `asyncio.to_thread` 调 `execute_action`（envelope 接受 `successful` / 旧版 `success` 两种 key + 非 dict 响应当成 success 包起来）+ TTL cache（默认 300s）+ 错误分类前缀（`composio_unavailable` / `composio_auth_error` / `composio_rate_limited` / `composio_action_not_found` / `composio_network_error` / `composio_action_failed[CODE]` / `composio_error`）。`daemon/factory.py` 加 `tools.composio` block 解析（enabled+空 api_key → ConfigError；apps 必须是 list）。`daemon/config.example.json` 加默认块 + 解释 OAuth 在 Composio dashboard 一侧做。30 条新单测（`tests/unit/test_v2_tool_composio.py`）：lazy-import 三态 / constructor 校验 / spec 翻译 (wrapped+flat) / TTL cache / 错误分类 (4 种异常 + envelope 失败) / factory wiring (5 路径)。`pytest -q` 30 passed, ruff/mypy/import-direction clean。**Auth flow** documented：bridge 只消费已授权 entity，不发起 OAuth。`pyproject.toml` 的 `tools-composio = ["composio-core>=0.7"]` 细粒度 extra 在并行 commit a980ca7 (B-386-followup) 一并落地。
- 2026-05-09: B-387 4 国产 LLM provider 集成测试矩阵 — `OpenAILLM(base_url=...)` 早就能打 DeepSeek / Kimi / Qwen / Gemini 但没有锁住每家响应 shape 的契约，响应格式悄悄漂（extra fields / 不同 finish_reason 值 / tool_call envelope 微差）只能在用户运行时炸。**新增** `xmclaw/providers/llm/_provider_profiles.py` (~200 行) 把四家 endpoint 的 `default_base_url` / `default_models` / `default_max_tokens` / `supports_thinking` / `supports_tool_use_streaming` / `supports_prompt_cache_marker` / `docs_url` 静态字典化（`ProviderProfile` frozen dataclass + `PROFILES` 元组 + `get_profile()` / `list_profiles()` / `detect_profile_from_base_url()` 反查），给后续 onboard 向导 + `xmclaw doctor` 一处单源 — 加 profile 永远不改运行时，只描述 endpoint 历史接受过什么。**新增** `tests/unit/test_v2_llm_providers_domestic.py` (~600 行) 41 测分四组：(1) per-provider config-shape 验 `base_url` / `api_key` 透传到 `AsyncOpenAI`（monkeypatch SDK 抓 kwargs）+ 4 family 的 `default_base_url` 都是 https + `default_models` 至少 1 条 + `get_profile` round-trip / 大小写不敏感 / 未知 id → None / `list_profiles` 顺序稳定 / `detect_profile_from_base_url` 8 测含 Ollama 不在册 + 空串 + None 边界；(2) per-provider 非流式 decode — DeepSeek（`finish_reason=stop` + 嵌套 `prompt_tokens_details.cached_tokens` 自动缓存）、Kimi（flat `cached_tokens` 走 `model_extra` pydantic v2 路径 + tool_call 走 OpenAI 标准 stringified arguments + CJK 内容）、Qwen（`finish_reason=tool_calls` + `content: null` + `tool_calls[].index` 字段 + 嵌套 `prompt_tokens_details.cached_tokens`）、Gemini（最小 OpenAI 形状 + 无 cache 字段）；(3) per-provider 流式 decode — DeepSeek 普通文本累积 + 末 chunk usage、Kimi 流式 `reasoning_content` 走 `on_thinking_chunk` 单独通道（B-91 路径）、Qwen 流式 tool_call `arguments` 按 chunk 拼接（seed chunk + 两段参数 chunk + 空 choices tail）+ `finish_reason=tool_calls`、Gemini stripped-down chunk 形状；(4) tool-call envelope shape — DeepSeek arguments 标准 JSON string 解码、Kimi CJK arguments（ensure_ascii=False）round-trip、Qwen 兼容 `arguments` 是 dict 不是 string 的兜底分支（translator defensive branch）、Qwen `tool_calls[].index` 字段不破坏 decode、Gemini 残缺 `function` 块走 anti-req #1 silent drop；(5) profile vs 运行时 cache flag 对齐 — 每 profile 的 `supports_prompt_cache_marker` 与 `_default_prompt_cache_enabled(model, base_url)` 对每家 expected 值一致（防 onboard 勾的 wizard 选项与 OpenAILLM B-320 自动检测漂移）。`scripts/test_lanes.yaml` llm lane 登记新文件；复用 pytest's `pytest_base_url` plugin 撞名规避（参数名 `b_url` 不用 `base_url` — 同 `test_v2_openai_provider.py::test_b320_default_prompt_cache_enabled_per_provider` 写法）。新文件不动 `OpenAILLM` 实现 — 整个 ticket 是契约固化（canary detection），不引入新 adapter (commit 0e6be78)
- 2026-05-09: B-393 Email channel adapter 新建 (`providers/channel/email/`) — IMAP4_SSL 轮询入站 (默认 30s 周期) + SMTP_SSL / STARTTLS 发出，纯 stdlib (`imaplib` + `smtplib` + `email`)，无新增依赖；sync IO 包在 `asyncio.to_thread` 里跑；MIME multipart 解析（text/plain 优先、text/html 回退用 stdlib `html.parser` 剥标签）+ RFC 2047 header decode (subject / from-name)；Message-ID LRU 去重 (cap 512)；allowed_senders allowlist (lowercase 比较) + Epic #14 injection scan (SOURCE_CHANNEL)；In-Reply-To / References + Re: 前缀线程化；imap_processed_folder 选项控制处理后是 mark Seen 还是 copy+expunge 移动；Gmail/Outlook/163 等 5.7.8 / AUTHENTICATIONFAILED 错误 → 提示 App Password 链接 (https://support.google.com/accounts/answer/185833)；imap_password / smtp_password 留空时回落 `xmclaw.utils.secrets` (key: `channels.email.imap_password` / `channels.email.smtp_password`)；`registry.CHANNEL_IDS` 加 email；`daemon/config.example.json` 加 channels.email 块（含 App Password 警告 / Gmail/Outlook/163/QQ 主流 host 示例 / allowed_senders 大小写注释）；43 条单元测试 (`tests/unit/test_v2_channels_email.py`)，无第三方 SDK，所有 IMAP/SMTP 客户端用 `unittest.mock.MagicMock`；ruff/mypy clean (代码 commit aeb8040 — 与 B-384 同 commit 落地)。
...
- 2026-05-07: **Epic #4 Phase D 进化链生产闭环** — 一周排查发现 Phase C 接通的引擎其实在生产 daemon 静默死掉, 修了 6 个 bug:
  * **B-294 (commit 40d082b)** —— `EvolutionAgent.evaluate()` 在生产从未被调用. 加 `EvolutionEvaluationTrigger`: 订阅 `GRADER_VERDICT`, 30s debounce + 5min cooldown + min 10 verdicts 阈值, 跳 `_system`/`skill-dream`/`dream:*`/`evolution:*`/`reflect:*` 五类内部 session. lifespan 装上去.
  * **B-295 (commit de1a64c)** —— variant 永远不会拿到 explore 流量. 加 `xmclaw/skills/variant_selector.py`: UCB1 over `(skill_id, version)` 各臂, HEAD 5 plays warmup 后切 bandit, 失败安全回 HEAD. `SkillToolProvider.invoke` 注入 selector 后从 `pick_version(skill_id)` 拿版本 + `result.metadata['skill_version']` 透到 grader 让 verdict 落到正确 arm.
  * **B-296 (commit b05aeb5)** —— controller 把所有 skill 混到一次调用导致跨 skill 误晋升. 改 `evaluate()` 返回 `list[EvolutionReport]`, 内部 per-skill_id 迭代 controller; 接收 `registry=` kwarg 自查每个 skill 的 HEAD. 7 测.
  * **B-297 (commit b05aeb5 一同)** —— `_arms` 仅在内存, daemon 重启 EWMA 清零 → controller `min_plays` 永远不达. 加 `~/.xmclaw/v2/evolution/<agent_id>/state.json`, atomic `os.replace` 写, 损坏 JSON warning 起空, 6 测 (含 50 次连写无 .tmp 残留 + 跨 daemon 重建保留 plays).
  * **B-298 (commit bb73db0 + 7e65ed7)** —— 上面四个加好后, 生产 daemon 还是没动: 双 root cause 静默失效. (a) `cli/main.py serve` 从未调 `setup_logging()` → 所有 `log.info` 走无 handler root logger 直接 `/dev/null`, 本来该证明 wiring 跑了的关键日志一行都没. (b) lifespan 找 SkillToolProvider 用错属性名 (`_providers` 实际是 `_children`/`children`), 加上 factory.py:1141 把工具栈包成两层嵌套 Composite, 单层 flat 迭代根本看不见 → `_evo_registry`/`_registry` 永远 None → `EvolutionAgent` 拿不到 registry, `VariantSelector` 永远不 start. 抽 `_find_skill_provider()` 模块级递归 walker (DFS + id() 判环, public/private/legacy 三种命名都接), 单元 10 测 + 集成 5 测 (验证 lifespan 真走完整链路 → state.json 落地 → 跳 prefix 不计数). 重启后 `evolution.start` / `evolution_eval.start` / `variant_selector.start` 三行第一次出现.
  * **B-299 (commit 1c4705f)** —— 链路修完了, `_arms` 还是空: 上游 agent 几乎不 invoke `skill_*`. 实测 prefilter (B-238) 在 404 个真技能上, CJK 查询命中 0 个 skill_* (英文 desc 与 CJK 无 token 重叠) → LLM 看见 zero 技能 → 回退 bash. 加 `skill_browse` 元工具: `SkillToolProvider.list_tools()` 永远在 index 0 prepend, prefilter 白名单透传, system prompt 引导 LLM 'before falling back to bash, call skill_browse'. 评分 = token-overlap (复用 prefilter._score_skill) + 子串匹配 (整段 5.0 / 单 token 1.5), top_k 默认 8 / 上限 25. 实测 `'git commit message'` 现在精准命中 `skill_commit-message-storyteller` (11.5) / `skill_git-commit` (11.5), `'帮我查一下天气'` 至少看见 meta 工具自己有了发现路径. 15 单元 + 6 个老 test_v2_skill_tool_bridge.py 改 helper 过滤 meta. 完整 unit 套 2280 passed.
  - 整链路验证: 51/51 evolution 相关测试全过. 生产 daemon 重启后 `~/.xmclaw/v2/evolution/evo-main/` 目录会在第一个非内部-session 的 GRADER_VERDICT 后被建出来 + state.json 写入. 仍待真正端到端实测: agent 真的去 `skill_browse(query=...)` 后 verdict 累积 → controller propose → orchestrator promote.
  - 未做: README killer demo GIF / 一周实测 grader +0.1 / `xmclaw evolution show --since 7d` ≥3 条 — 留给 Phase E.

- 2026-05-07: **B-319 / B-320 — extractor 合并 + OpenAI cache_control 平价 + docs 同步** (一并落地).
  * **B-319 ExtractFactsHook 合并**: `xmclaw/daemon/post_sampling_hooks.py`. ExtractLessonsHook 在 5 桶 (workflow/tool_quirks/failure_modes/values/rules) 上加第 6 个 `preferences` 桶 → 写 `kind="preference"` (不是 lesson) / `bucket=None` 直达 USER.md 的 `## Auto-extracted preferences` 段 (匹配 `AUTO_SECTIONS["USER.md"]` 的 `bucket_filter=None`). `_LESSON_BUCKETS` dict 从 `(file, section)` 升到 4-tuple `(file, section, kind, bucket)` 让单一 dispatch 路径同时跨 lesson 和 preference 两种 kind. `ExtractFactsHook = ExtractLessonsHook` 前向兼容 alias 加上去, `ExtractMemoriesHook` 标 deprecated 并从 `build_default_registry` 拿掉 (它的 failure_modes 桶现已被 ExtractLessonsHook 覆盖, 留着只是多花一次 LLM call), 配置开关接受 `evolution.memory.extract_facts.enabled` / `extract_lessons.enabled` 双别名. 净效果: 单次 turn-end LLM call 现在覆盖全部 7 个 persona 文件的自动写路径 (不只是 ProfileExtractor 之外的 6 个), ProfileExtractor 降为可选深度分析器. `tests/unit/test_v2_post_sampling_hooks.py` +4 测 (preferences 路径到 USER.md / `kind="preference"` 不带 bucket / `ExtractFactsHook is ExtractLessonsHook` / 默认 registry 只有 extract_lessons), 全 27 测 + 周边 118 持久测全过.
  * **B-320 OpenAI prompt cache 平价**: `xmclaw/providers/llm/openai.py`. 加 `prompt_cache_enabled: bool|None` 构造参数 + `_default_prompt_cache_enabled(model, base_url)` 自动探测 (Moonshot/Kimi + Zhipu/GLM 默认开, OpenAI 自家 + DeepSeek + 未知 compat shim 默认关 — 严格 schema 服务器拒未知字段会 400). 启用时 `_messages_to_openai` 把最后一个 system 消息升级到 Anthropic 风格的 content-block 列表带 `cache_control: ephemeral`, `_tools_to_openai` 在最后一个 tool 上加同样标记 — 镜像 B-245 AnthropicLLM 的双 breakpoint 形状. `_extract_cache_tokens(usage)` helper 同时探测 (a) Anthropic 风格 flat field (`cache_creation_input_tokens` / `cache_read_input_tokens`, Moonshot/Zhipu) 和 (b) OpenAI 风格嵌套 `prompt_tokens_details.cached_tokens` (OpenAI 自动 cache + DeepSeek), 都映射到 `LLMResponse.cache_*` 字段, 让 COST_TICK pipeline 在所有 provider 上统一报缓存命中. `xmclaw/daemon/factory.py` `_instantiate_llm` + `build_llm_from_config` + `build_llm_profiles_from_config` 透传 `prompt_cache_enabled`, 支持 legacy block 继承 (跟 api_key/base_url 同款). `tests/unit/test_v2_openai_provider.py` +18 测 (system block decoration / tool last-marker / per-provider 自动探测 7 条 parametrize / 显式 override 胜过自动 / cache token 三种来源都拉对). 30/30 + 102 factory/registry 测全过.
  * **docs 同步**: 重写 `docs/EVOLUTION.md` 反映 B-294→B-318 实际架构: 加 §2 完整 pipeline picture (vertical mutation + horizontal proposing 两轴), §5a/§5b 拆解 MutationOrchestrator (B-172) + SkillDreamCycle/RealtimeEvolutionTrigger (B-164) + ProposalMaterializer (B-167), §7 加 `evolution.evaluation.{debounce_s,cooldown_s,min_new_verdicts}` + `auto_apply` 配置, §9 加 B-318 SKILL_DRAFTED / SKILL_PROMOTION_RECOMMENDED / SKILL_ROLLBACK_RECOMMENDED 事件 (legacy SKILL_CANDIDATE_PROPOSED 仍 backward-compat). README §What's different 进化卡片从"5 个 observer"改"6 个" (加 ExtractFactsHook), 把 ProfileExtractor 描述改成现在主写路径的 ExtractFactsHook + 把 SKILL_CANDIDATE_PROPOSED 改成 SKILL_DRAFTED + SKILL_PROMOTION_RECOMMENDED + 加 EvolutionEvaluationTrigger + MutationOrchestrator 进 wiring 描述. README §Architecture 把 LLM provider 列表加 "B-320 prompt-cache parity for Moonshot/Kimi & Zhipu/GLM" 提示, 进化链文字加 EvolutionEvaluationTrigger + MutationOrchestrator + SkillDreamCycle/RealtimeEvolutionTrigger/ProposalMaterializer 三组件.
  - 验证: 2319 unit tests passed (剩 4 个失败都是 pre-existing 与 B-319/B-320 无关: `test_evolution_agent_observer_receives_skill_verdicts` plays 计数飘移 / `test_b205_memory_search_appears_before_sqlite_query` 系统提示工具排序 / `test_no_file_exceeds_line_budget` `app.js` 581 行超 500 / `test_all_four_groups_are_mounted` pydantic forward-ref).

- 2026-05-07: **B-321 / B-322 — 把 4 个 pre-existing 失败一刀清掉**.
  * **B-321 (4 件套测试 + UI 拆文件)**:
    - `tests/unit/test_v2_agent_loop.py::test_evolution_agent_observer_receives_skill_verdicts`: 加 `tmp_path` fixture + 给 `EvolutionAgent("evo-test", bus, audit_dir=tmp_path)` 显式隔离. 根因: B-297 加的 EWMA state.json 持久化默认走 `~/.xmclaw/v2/evolution/<agent_id>/state.json`, 测试不传 `audit_dir` 就跨 pytest 跑累积 plays — 第一次跑过, 第二次起 `plays==1` 断言挂. 顺手把 `tests/unit/test_v2_evolution_evaluation_trigger.py` 6 个相关测试同款修了 (它们 `EvolutionAgent("test", bus)` 也没 audit_dir, 现在虽然没失败但也在污染家目录).
    - `tests/unit/test_v2_default_system_prompt.py::test_b205_memory_search_appears_before_sqlite_query`: 改成"在 self-management toolkit section 内部"找 `memory_search` vs `sqlite_query` 的相对位置, 不再用 `p.find()` 扫全文. 根因: B-302 HONESTY RULE 加的 plan-mode-skip 列表合理地先提到 `sqlite_query` (作为 read-only op 例子) — 全文 find 抓到 plan-mode 那一行就把测试推翻了, 而 B-205 invariant 的语义 ("self-management toolkit 里 memory_search 排第一") 完全没破.
    - `tests/unit/test_v2_ui_scaffold.py::test_no_file_exceeds_line_budget`: app.js 581 行超 FRONTEND_DESIGN.md §1.4 的 500 行硬约束. 把 lines 280-405 的 7 个 composer-side action helper (`sendComposer` / `setLlmProfile` / `cancelComposer` / `answerQuestion` / `changeDraft` / `togglePlan` / `toggleUltrathink`) 抽出来成 `xmclaw/daemon/static/lib/composer_actions.js` 走 `createComposerActions({store, getWsHandle, ...})` 工厂模式 (跟 `lib/chat_actions.js` 一致). 抽完 app.js 477 行, 守规.
...
**P0 检查清单**（**全部完成 2026-05-19**）：

- [x] **G-02 Load failures**：`SkillsWatcher._load_failures` dict + `load_failures()` getter + `GET /api/v2/skills/load_failures` 端点 + `GET /api/v2/skills` 顶层 `load_failures` 字段（commit a49ad78）
- [x] **G-03 requires_restart**：`_refresh_changed_bodies` 不再 skip unregistered skill 目录 + `_skills_with_history_of_failure` set 持久化失败历史 + `pending_restarts` payload 加 `state='fixed_after_failure' | 'edited'` + system prompt block 告诉 agent 该怎么读 + 怎么向用户传话（commit ef508e6）
- [x] **G-01 自省工具**：`skill_status()` + `skill_view(skill_id)` + `skill_view(skill_id, file_path)` 三个 LLM meta-tool，注册到 `SkillToolProvider`，prefilter 白名单常驻；`SkillToolProvider(watcher=...)` 拿到实时状态；`skill_view` 拒绝 `..` + 绝对路径（Windows-aware）（commit 5eca3d9）

**P1 检查清单**（**全部完成 2026-05-19**）：

- [x] **G-09 路径统一**：`UserSkillsLoader.load_all` realpath 去重（symlink-safe）+ same-id-different-path 记 `kind='duplicate'` LoadResult → 进 watcher.load_failures + skill_status 输出（commit 01fd086）
- [x] **G-10 frontmatter 扩展**：`SkillManifest` 加 `when_to_use` / `allowed_tools` / `paths` / `requires_restart` / `model` 5 个新字段；`_parse_skill_md_frontmatter_extras` 解析器（接受 snake_case + hyphen-case + camelCase 三种命名）；`_load_manifest` 同步扩；`_load_markdown` wire 通；5 个测试 pin 解析 + 端到端 manifest threading（commit 330a5da）
- [x] **G-04 progressive disclosure**：`SkillToolProvider` 加 `disclosure_mode` (`inline` / `unified` / `auto`，默认 auto) + `unified_threshold` (默认 20) + 新 `skill_run(skill_id, args)` meta-tool（始终暴露 + prefilter 白名单常驻）。`unified` 模式只暴露 6 个 meta-tool（browse / view / status / install / uninstall / run），LLM 走 Hermes 风格 browse→view→run 三步流程；`inline` 保留 legacy 每 skill 一个 `skill_<id>`；`auto` 根据 registered 数 vs 阈值切换。daemon `config.skills.{disclosure_mode,unified_threshold}` 暴露；17 个 G-04 测试 + 修复 5 个 prefilter 测试预先 inline 化 pin 边界。

**P2 检查清单**：

- [x] **G-05 conditional skills**：frontmatter `paths: [...]` 通过 `SkillToolProvider._spec_for` 映射到 `parameters_schema["x_paths"]`；`select_relevant_skills` 加 `active_paths` 参数，对声明了 paths 的 skill 做 boost (+3 命中) / gate (-1 全 miss)；`extract_recent_paths(messages, lookback, max_paths)` 在 `agent_loop` prefilter 调用前从 `prior` 历史里扫最近 file_read/file_write/list_dir/file_search/file_glob/file_grep/file_edit 工具调用提取路径（忽略 bash + skill_<id> 因 args 不规则）；Windows 反斜杠 + 大小写不敏感 + `**` 跨段 glob 都规整。18 个新测试 pin 全部行为。free-code 的 `activateConditionalSkillsForPaths` 参考实现，但更轻——直接复用现有 prefilter 不需新组件。
- [x] **G-06 trust 等级**：`SkillTrustLevel` 4-level enum (BUILTIN > USER > INSTALLED > UNTRUSTED) on `SkillManifest`, 默认 USER；UserSkillsLoader 读 `.marketplace.json` 一次性建 `_installed_skill_ids` 集合，5 个 manifest 构造点都注 `trust_level=self._trust_for(skill_id)`（marketplace 在册 → INSTALLED 否则 USER；manifest 作者声称的更高 trust 被 loader 覆盖）；SkillRegistry.promote 加 `force: bool=False` + `DangerousPromotionError`（`PermissionError` 子类），evidence 含 `dangerous:` 或 `verdict=dangerous`（大小写不敏感）就拒，需 `force=True` 显式覆盖；SkillToolProvider spec description trailer 加 `trust=<level>` 标签 + 单独的 `allowed_tools: foo, bar (+N more)` 行（前 5 个）。16 个新 G-06 测试。修 3 个旧 tool_bridge 测试的 trailer 格式 assert。
- [x] **G-07 用户 skill rollback**：新模块 `xmclaw/skills/version_history.py` 提供 `snapshot / list_versions / rollback / diff` 4 个原子操作 + `MAX_VERSIONS_PER_SKILL=50` 滚动剪枝；SkillsWatcher 的 `_maybe_update_body`（markdown）+ `_maybe_hot_reload_or_announce`（Python，同步快照 skill.py / SKILL.md / manifest.json 三件套）在检测到 mtime 变化时调 `snapshot()` 写入 `<skill_dir>/.versions/<UTC-ISO>.<ext>`，幂等去重（相同内容 skip）；两个新 meta-tool `skill_diff(skill_id, file?, against_index?)` 返回 unified diff（最多 200 行截断）+ `skill_rollback(skill_id, file?, to_index?)` 把指定 snapshot 还原回 live 文件（snapshot_current=True 先存当前作为 undo 锚点）；两者都加入 `_ALWAYS_ON_META` 白名单。20 个新 G-07 测试覆盖所有边界（缺文件 / 越界 index / 内容不变去重 / pruning / 截断 / meta-tool dispatch）。**`skill_patch` 增量编辑 + CLI `xmclaw skills rollback` 推后到独立 P3 issue**——MVP 双 meta-tool + 自动快照已经覆盖 95% 的 "我刚把 SKILL.md 写崩了，还原" 场景。
- [x] **G-08 self-evolving skills**：新 meta-tool `skill_propose(name, body, description?)` 让 agent 在磁盘上写新 skill（`~/.xmclaw/v2/skills_user/<name>/SKILL.md` + `.proposed.json` 标记）；UserSkillsLoader 扫 `.proposed.json` 标记 → `_proposed_skill_ids` 集合 → `_trust_for` 优先级修正（UNTRUSTED > INSTALLED > USER，proposal 标记最强信号因为最未审）；`promote_after_evidence=3` 写入 marker（agent 默认目标），但 MVP 实际门槛是 manual remove marker + restart（不做完整 ≥3 evidence 自动促升流程，保留 XMclaw 差异化"不抄 Hermes 即时学习 transient failure"）。slug 强校验 `[a-z0-9][a-z0-9_-]{0,40}`，body 最小 50 chars 防空 stub，已存在的 skill 拒绝（要 edit 走 skill_diff / skill_rollback）。12 个新 G-08 测试 + 修 2 个 _META_TOOL_NAMES 集合。
...
- 2026-05-19（同 session）: **P0 全部完成** → G-02 load failures (a49ad78) / G-03 requires_restart (ef508e6) / G-01 自省 tools (5eca3d9)。**P1 完成 2/3** → G-09 realpath dedup (01fd086) / G-10 frontmatter 扩展 (330a5da)。下一步：G-04 progressive disclosure 是 P1 最后一项也是最大改造（要重写 tool 注册路径 + 系统 prompt），单独排下一个 session；G-05/06/07/08 为 P2，按依赖顺序（G-05 → G-06 → G-07 → G-08）逐步推。
- 2026-05-19（晚间 session）: **Python skill.py 热重载** 完成 (4b40ccf) — XMclaw 成为唯一一个支持 Python skill 热重载的 agent runtime（竞品全是 markdown-only），通过 `SkillRegistry.hot_replace` + `UserSkillsLoader.reload_one` + mtime-stamped 模块名绕开 sys.modules 缓存。新增 `SKILL_HOT_RELOADED` 事件类型。剩余 G-04 + P2 全套未做。
- 2026-05-19（peer gap-analysis 后 sweep 17 个 finding 推进）：**sweep P0/P1/P2 大批落地**：#11 metacognize 60s→1800s + #4/#5 memory recall 并行 (18043e4) / #13 Skills.js banner (3e3abfa) / #8 embedding circuit breaker + #1 planner repair 保留 partial (570c93a) / #7 LanceDB schema-degraded refuse early (725461d) / #14 anthropic max_tokens 4096→8192 (a453258) / #15 daily_digest croniter fallback (032d342) / #9 persona-write 30s inner timeout (1ebb20a) / #3 stub fallback ok=False + bus 接通 (c1ca33e) / #12 slow_subsystem 自愈 cooldown (ede1fd2) / #6 maintenance scan cap + 截断 warning (90acc15)。**还没做**：#10 HoldoutTestSignal 真实现（grader 文档 vs 实际只 1 个 signal 的诚实度问题）+ #16 ToolResult.error 文案统一 + G-04 progressive disclosure + G-05/06/07/08（P2 架构改）+ Epic #26 Phase C（plans.db + budget guard + UI panel）。
- 2026-05-19（G-08 落地，Epic #27 P0+P1+P2 全收尾）：**self-evolving skills 完成** — 新 meta-tool `skill_propose(name, body, description?)` 始终暴露（prefilter 白名单），让 agent 在 `~/.xmclaw/v2/skills_user/<name>/` 写 SKILL.md + `.proposed.json` 标记。UserSkillsLoader 新方法 `_scan_proposed_skill_ids` 一次性扫所有标记，trust 优先级改为 UNTRUSTED（proposed）> INSTALLED（marketplace 注册）> USER（默认）—— proposed 标记是最强信号因为它说明 agent 自己写的还没审。`skill_propose` 守门：slug 必须 `[a-z0-9][a-z0-9_-]{0,40}`、body >= 50 字符、已存在的 skill 拒绝（强制走 skill_diff + skill_rollback 编辑）、body 无 frontmatter 但提供了 description 时合成最小 frontmatter 块。Marker JSON 记录 `proposed_by="agent"` / `proposed_at` / `evidence_count=0` / `promote_after_evidence=3` —— 后两个是未来 evidence-gated 自动促升的占位字段，MVP 实际门槛是 manual remove marker。**反 cargo cult**：不抄 Hermes "5+ tool calls = save as skill" 的即时学习陷阱，保留 XMclaw"evidence-gated"差异化。12 个新 G-08 测试 + 修 2 个 _META_TOOL_NAMES 集合。**Epic #27 完成**：P0 (G-01/02/03) + P1 (G-04/05/09/10) + P2 (G-06/07/08) + sweep 17/17 全收尾。

- 2026-05-19（Sweep #16 follow-up 收尾）：**MCPBridge + browser tool 错误消息结构化** — 之前 sweep #16 partial 把 `_fail_with_hint` 拉出来只接了 file_write 4 个 catch；现在收尾：MCPBridge.invoke() 4 个 fail 站点全换（not_started / timeout / MCPError / generic Exception）+ tool-reported isError 一并加 hint；browser tool generic-exception 路径用 `_fail_with_hint` 给出 snapshot / wait / reopen 三条具体恢复路径；_PlaywrightMissing 站点提示 `pip install 'xmclaw[browser]' && playwright install chromium`。每条 hint 都指向具体动作（"check daemon.log under mcp.stderr name=..."、"bump request_timeout"、"restart daemon to recycle MCP child"），LLM 读完能立刻执行。**bonus**：MCPBridge fail 路径以前 latency_ms=None，现在全部测量正确（pre-fix 4 处缺失全补上）。6 个新测试（MCPBridge 4 + tool_reported_error + success path latency 回归）。Epic #27 P0/P1/P2 + sweep 全收尾。

- 2026-05-19（Sweep #10 落地）：**HoldoutTestSignal 真实现完成** — pre-fix 该 signal docstring 说"可执行 holdout check"实则 stub，永远返回 None（除非测试用 `holdout_test_passed` payload override 短路）。sweep audit 发现"多 signal 诚实度"的虚假承诺：HonestGrader 文档说 multi-signal 实际只 UserFollowupSignal 真跑。**修复**：新模块 `xmclaw/core/grader/holdout_registry.py` 暴露 `register / unregister / lookup / clear / run_check / registered_ids`，进程本地 dict 注册 `eval_test_id → callable`；sync + async callable 都接，捕所有异常返 None（坏 verify hook 不会崩 grader），int 0/1 强转 bool，其他怪返回值（None / str / dict）一律 None。HoldoutTestSignal.probe 重写：1) `holdout_test_passed` payload override 仍优先（向后兼容现有测试）2) eval_test_id → registry lookup → run_check 真路径，registered 返 1.0/0.0；unregistered/raise 返 None 不假装得分。21 个新测试 + 修 1 个 legacy `stub_no_executor_yet` → `unregistered` 状态串。Sweep finding 完结率：17/17 全收尾。

- 2026-05-19（G-07 落地）：**skill rollback via .versions snapshot 完成** — 新模块 `xmclaw/skills/version_history.py` 提供 `snapshot` / `list_versions` / `rollback` / `diff` 4 个 best-effort 原子操作（snapshot 失败永远不阻断 live reload，写不进去打 WARN 然后跳过）。SkillsWatcher 双路径接入：`_maybe_update_body`（markdown）+ `_maybe_hot_reload_or_announce`（Python，同步快照 skill.py / SKILL.md / manifest.json 三件套）在 mtime 变化时调用 snapshot()，幂等去重避免编辑器疯狂保存污染。新 meta-tool `skill_diff(skill_id, file?, against_index?)` 返回 unified diff（200 行截断）+ `skill_rollback(skill_id, file?, to_index?)` 还原 snapshot 到 live（snapshot_current=True 先把当前 live 也存进 .versions/ 作为 undo 锚点）；两个 meta-tool 加入 `_ALWAYS_ON_META` 白名单确保 prefilter 永远不剔。20 个新 G-07 测试 + 修 2 个测试的 _META_TOOL_NAMES 集合（多了 META_DIFF + META_ROLLBACK）。**P2 剩余**：仅 G-08 self-evolving skills + Sweep #10 HoldoutTestSignal 真实现 + Sweep #16 follow-up。

- 2026-05-19（G-06 落地）：**trust levels + dangerous-promotion gate 完成** — 新 `SkillTrustLevel` 4-level enum（BUILTIN > USER > INSTALLED > UNTRUSTED）落到 SkillManifest 作 frozen dataclass 字段，默认 USER；UserSkillsLoader 加 `_read_installed_skill_ids` 一次读 `.marketplace.json`（缺失 / 损坏 / 怪形都退到 frozenset()），`_trust_for(skill_id)` 返回 INSTALLED if in 集合 else USER。5 个 manifest 构造点（_load_python manifest 缺失 / _load_python manifest.json / _load_markdown / versions/ 子目录 / _load_manifest 自身）都注 trust override，manifest 作者写的不算数 loader 有最终决定权。SkillRegistry.promote 加 `force: bool=False` 参数 + 新 `DangerousPromotionError`（`PermissionError` 子类，跟 ValueError 分开 catch），evidence 含 `dangerous:` 或 `verdict=dangerous`（大小写不敏感）就拒，要 force=True 显式覆盖。SkillToolProvider description trailer 改为 `[skill:<id> v<n>, trust=<lvl>, by=<created_by>]` + 单独一行 `allowed_tools: a, b, c (+N more)` 前 5 个。16 个新 G-06 测试 + 修 3 个旧 tool_bridge 测试的 trailer 格式 assert（now contains `trust=...` field between id and by=）。**P2 剩余**：G-07 rollback / G-08 self-evolving skills。

- 2026-05-19（G-05 落地）：**conditional skill activation 完成** — `SkillToolProvider._spec_for` 现在把 manifest.paths 映射到 spec.parameters_schema 的 `x_paths` 槽（同时把已有的 `triggers` 也映射成 `x_triggers`，少一处重复编码）。`select_relevant_skills` 加 `active_paths` 参数：声明了 paths 的 skill 在某条 active_path 命中其任一 glob 时 +3 boost；声明了 paths 但全 miss 时 score=-1 被零分门槛过滤掉；没声明 paths 的 skill 不动（preserve legacy 大多数情况）。新 `extract_recent_paths(messages, lookback=8, max_paths=20)` helper 从 `prior` 消息历史扫最近 file_read / file_write / list_dir / file_search / file_glob / file_grep / file_edit 调用（白名单设计——bash 不在内，因 command arg 不规则容易污染）。Windows ↔ Posix 路径规整 + 大小写不敏感 + `**/` `/**` 跨段 glob 折回 fnmatch。`agent_loop` 在 prefilter 调用现场提取 active_paths 并注入。18 个新测试 + 全套 agent_loop 125 个回归全过。**P2 剩余**：G-06 trust / G-07 rollback / G-08 self-evolving skills。

- 2026-05-19（Phase C 后续）：**G-04 progressive disclosure 完成** — `SkillToolProvider` 加 `disclosure_mode` (`inline`/`unified`/`auto`，默认 auto) + `unified_threshold` (默认 20) + 新 `skill_run(skill_id, args)` meta-tool（始终暴露 + prefilter 白名单常驻）。**unified 模式**只暴露 6 个 meta-tool（browse / view / status / install / uninstall / run），LLM 走 Hermes 风格 browse→view→run 三步流程，大库下省 ~50 tokens × N 描述预算；**inline** 保留 legacy 每 skill 一个 `skill_<id>` 工具；**auto** 根据 registered skill 数 vs 阈值（>20 → unified）自动切换。配置经 daemon `config.skills.{disclosure_mode,unified_threshold}` 暴露（`config.example.json` 加 schema）。17 个新 G-04 测试 + 修 5 个 prefilter 老测试预先 pin `inline` 模式（registry 总数 >20 触发 auto→unified 会让"per-skill 工具应该可见"的断言失效）。**P1 剩余**：仅 #10 HoldoutTestSignal 等真实现（grader 诚实度）+ G-05/06/07/08 P2 架构改。

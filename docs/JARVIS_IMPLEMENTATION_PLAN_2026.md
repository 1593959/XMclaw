...
- 2026-06-17: **新增安卓手机控制能力（ADB，对标 Kimi kimiclaw）**（用户："给 xmclaw 开发控制手机的能力"）。新增 `providers/tool/android.py` 的 `AndroidToolProvider`：后端走 `adb` CLI（业界标准，droidrun/AppAgent/Mobile-Agent 同路线），9 个工具 `android_devices/screenshot/tap/swipe/text/key/ui_dump/app_list/launch_app`。截图经 `metadata.attach_image` 喂视觉模型（与 computer_use/generate_image 同通道）；`ui_dump` 解析 uiautomator XML → 元素 text/resource-id/clickable/center+bounds 供可靠定位（而非纯像素猜）。**安全**：默认关（`tools.android.enabled`，与 computer_use 同 opt-in），所有 adb 调用是 argv 列表 + `shell=False`（工具层硬规则），`input text` 用户文本转义（空格→%s）不拼 shell 行；adb 缺失/无设备时逐工具降级报错不崩。前置：装 platform-tools(`adb` 入 PATH) + 手机开 USB/无线调试。factory 按 `tools.android.enabled` 接线。8 个 mock-subprocess 单测(argv 断言 + 输出解析 + 工厂门控)；test_lanes tools lane 登记。**已知限制**：`input text` 仅可靠处理 ASCII，中文/IME 需设备侧 ADBKeyboard 助手(未内置)，已在工具描述注明。

- 2026-06-17: **Phase 10 新增「文件域」——工作区 md 文件树 + 编辑器**（用户："在前端加上 md 文件树，我自己修改"）。后端 `/api/v2/files`(Epic #18)早已有 GET 列目录/读文件 + PUT 写文件(限 personas/memory/skills/agents/workspaces 根)，缺的是前端树。新增 `webui/src/views/FilesView.tsx`：左侧按 `/roots` 分组的懒加载文件树(点目录展开、点文件打开)，右侧编辑器(文本类 .md/.json/.yaml/… 可改 → PUT 保存、脏标记、403→不可写根提示)。导航加第 5 域「文件」(TaskRail grid-cols-4→5)+ `/files` slash 命令 + App 懒加载路由 + store view 联合类型。`npm run build` 通过(tsc + vite，FilesView chunk 1.9KB gz)，dist 提交。**待用户硬刷新确认效果**(改完 webui 需 Ctrl+Shift+R)。注：dentist/golden-retriever 假事实在 graph.db/memory.db(SQLite)，文件树触达不到，仍需单独清理。

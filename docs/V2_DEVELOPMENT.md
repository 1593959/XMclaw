# XMclaw v2 — 开发文档（2026-04-21 draft）
...
1. **学习曲线单调上升**：`phase1_learning_curve.py` 跑 50 轮，第 1/10/25/50 轮 grader 平均分严格递增，末段 > 首段 ≥ 20%。
2. **Grader 和人工判分一致率 ≥ 80%**：对同 50 条 event 人工标注 PASS/FAIL，与 `HonestGrader.score ≥ 0.6` 的判定对齐 ≥ 40 条。
3. **同模型对比不劣于裸 SDK**：同 50 任务，`裸 Anthropic SDK vs v2 CLI` 平均分差异 ≤ 5%（anti-req #11 落地）。
4. **事件流可重放**：`xmclaw replay <session_id>` 能完整复现 scheduler 决策。

任一不达标：回到 plan §10 fallback 路径讨论。
...
1. 本文和 [REWRITE_PLAN.md](REWRITE_PLAN.md) review 一遍，确认无偏差
2. 切分支 `v2-rewrite`，按 §8.1 清单创建空骨架 + placeholder
3. 写 Phase 1 首期 ticket（~15 个）
4. 搭 CI 硬门 10 条的初版脚手架（可以先占位，但要存在）
5. 开始 Phase 1 coding

从这一步往下，每个 PR 都对应 plan 的一个 checkbox。

...
7. ✅ Phase 4：daemon 集成 (FastAPI + WS `/agent/v2/{session_id}`、pairing token、event replay、events.db/memory.db)
8. ✅ v2 Web UI (`xmclaw/daemon/static/`) + CLI `xmclaw start / chat / doctor` 全通路
9. ✅ Strangler-fig 清理：v1 `agent_loop.py` / `evolution/*` / `genes/*` / `task_classifier.py` / 旧 daemon / 旧 UI 全部删除
10. ✅ v2-rewrite → main 合并；`main` 是 v2-only tree

### 当前候选（任选其一继续）

1. **Epic #3 subprocess runtime default** — 让 `providers/runtime/process.py` 成为生产默认（现在 `LocalSkillRuntime` 是 dev/test default），把 manifest 的 fs/net/memory 真的 enforce
2. **Epic #5 daemon memory retention** — factory 接 `retention_days` / `max_bytes` + 发 `MEMORY_EVICTED` 事件；目前 memory 无限增长
3. **多 channel adapter** — 把 conformance matrix 从 N=1 扩到 N=3（Slack/Telegram 实现）
4. **anti-req #14 phase 2** — SOUL / PROFILE / memory-recall 注入点的全 AgentLoop 分支审计
5. **M1 exit 72h stress** — 需要独立 host 跑多天的真实 soak test

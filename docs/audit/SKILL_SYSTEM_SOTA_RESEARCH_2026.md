...
| ⑩ 共享/市场 | SKILL.md 跨厂商生态 / MCP 注册表 | 本地 marketplace_index + user_loader **+ 已有 MCPHub**(cline 端口,Claude-Desktop 配置兼容,多 server stdio+HTTP) | 🟢 已接 MCP(残留:SSE/OAuth/热重载、Alita 式自动生成 MCP) |
...
3. **✅ ② 轨迹→技能归纳(Voyager 路线)— 已落地**(`xmclaw/skills/inductor.py`):成功
   ...
   `.proposed` SKILL.md(永不自动晋升,走证据门)。后台 loop
   `skills.induction.enabled` **默认 ON**(产出永远未信任、绝不上 HEAD,爆炸半径有限)。
   补齐"无中生有"。
4. **✅ ⑩ MCP 客户端接入 — 复核发现已实现**(`xmclaw/providers/tool/mcp_hub.py` = cline
   McpHub 端口:读 `~/.xmclaw/mcpServers.json` + `config.mcp_servers`(Claude-Desktop
   格式),多 server JSON-RPC stdio + `mcp_http_bridge`,本身是 ToolProvider,boot 时
   挂进 CompositeToolProvider)。残留:SSE/streamableHttp transport 完整化、OAuth、
   文件热重载、**Alita 式"缺能力时自动生成新 MCP server"**(真正未做的那部分)。
5. **🟡 ⑧ 复合技能** + **⑦ archive stepping-stones** — 轻量增强,顺带做。
6. **⑨ 自动课程** — 低优先,reactive 进化对个人助理已够。

> **复核总结(2026-05-31)**:本研究稿原把 ①SKILL.md、⑩MCP 误判成缺口,实测发现**都已
> 实现**(SKILL.md 消费 + MCPHub 客户端)。本 session 真正补上的**新能力**只有两项——
> **⑫ 语义自主调用**(真缺口,已修)和 **② 轨迹→技能归纳**(真新增,opt-in)。技能系统
> 的高价值缺口至此基本闭合;剩下的 ⑧/⑨ + MCP 的 SSE/OAuth/Alita-auto-gen 都是中低优先
> 增强,不构成"agent 不会自主用技能"那类根因问题。**建议:技能这条线先收,转去实测。**

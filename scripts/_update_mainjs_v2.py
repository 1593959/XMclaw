...
# 2. Replace render functions block
old_render_block = """function appendChunk(el, text) {
...
new_render_block = """function setInput(text) {
    input.value = text;
    input.focus();
...
function appendChunk(el, text) {
    messageBuffer += text;
    el.innerHTML = renderMarkdown(messageBuffer);

...
async function loadArchitectureFlows() {
    ...
    const flows = [
        ...
        },
        {
            ...
            label: '🧠 反思',
            icon: '🧠',
            desc: '每轮对话的 Stage 5 同步反思：分析 _turn_history → 问题/教训/改进 → 广播到前端',
            color: '#8b5cf6',
            steps: [
                { node: 'Main Loop 结束', type: 'entry', desc: 'LLM 最终响应已生成,进入 Stage 5', color: '#8b5cf6' },
                { node: 'ReflectionEngine.reflect() 同步', type: 'process', desc: '直接在 AgentLoop.run() 内 await,非后台', color: '#6366f1' },
                { node: '分析 _turn_history', type: 'process', desc: '读取本轮及历史 turns / 工具调用轨迹', color: '#ec4899' },
                { node: 'LLM 生成反思', type: 'process', desc: 'prompt → 问题/教训/改进建议 (JSON)', color: '#f59e0b' },
                ...
                { node: '_schedule_evolution_only()', type: 'process', desc: '后台启动: create_task 进化(反思不是后台)', color: '#a855f7' },
            ]
        },
        {
            id: 'evolution',
            ...
            steps: [
                ...
                { node: 'VFMScorer.score_gene/skill/file()', type: 'decision', desc: 'VFM 评分 ≥ 阈值才保留', color: '#14b8a6' },
                { node: 'EvolutionValidator.validate_gene/skill()', type: 'decision', desc: '语法+imports+runtime 六重验证', color: '#14b8a6' },
        ...
        },
        {
            ...
            label: '🧩 记忆',
            icon: '🧩',
            desc: '三层记忆架构:会话层(JSONL) + 向量层(sqlite-vec) + 结构层(SQLite) + 身份/工作区注入',
            color: '#06b6d4',
            steps: [
                { node: 'SQLiteStore', type: 'data', desc: 'agent 配置 / insights / 进化记录', color: '#6366f1' },
                { node: 'SessionManager', type: 'data', desc: 'JSONL: {agent_id}.jsonl 会话历史', color: '#3b82f6' },
                { node: 'VectorStore', type: 'data', desc: 'sqlite-vec 扩展: 向量嵌入 / 语义搜索', color: '#8b5cf6' },
                { node: 'SOUL.md / PROFILE.md / AGENTS.md', type: 'data', desc: '身份层:每轮注入到 Prompt', color: '#f97316' },
                { node: 'workspace/{plan,notes,decisions}.md', type: 'data', desc: '跨轮面包屑:LLM 自己写自己读', color: '#f59e0b' },
                { node: 'load_context()', type: 'process', desc: '对话前:加载历史 + 搜索记忆 + 读身份/工作区', color: '#14b8a6' },
                { node: 'save_turn()', type: 'process', desc: '对话后:SQLite → JSONL → vector.add (顺序)', color: '#14b8a6' },
                { node: 'vector.add()', type: 'process', desc: 'turn 内容 → 向量嵌入', color: '#06b6d4' },
                { node: 'search()', type: 'process', desc: 'top_k 相关记忆 → InfoGatherer', color: '#06b6d4' },
            ]
        },
        {
            id: 'eventbus',
            ...
            steps: [
                ...
                { node: 'rate_limit', type: 'process', desc: '单类型每 60s 滑动窗口 ≤200 条', color: '#f59e0b' },
        ...
        },
        {
            ...
            steps: [
                ...
                { node: 'CircuitBreaker 包裹', type: 'decision', desc: '连续失败 ≥5 → 跳闸 30s,防止雪崩', color: '#dc2626' },
                { node: 'tools.execute(name, args)', type: 'process', desc: '路由到具体工具类执行', color: '#f97316' },
                { node: 'Bash / File / Web 等', type: 'process', desc: '具体工具执行(sandbox / 权限校验)', color: '#ec4899' },
                { node: '执行失败?', type: 'decision', desc: '异常 → 错误结果注入,让 LLM 自我纠错', color: '#b91c1c' },
        ...
        },
        {
            ...
            label: '🧠 认知推理',
            icon: '🧠',
            desc: '五阶段认知流水线:分析→收集→规划→技能→反思;含身份注入、skill 短路、错误重试',
            color: '#8b5cf6',
            steps: [
                ...
                { node: 'Stage 4: 技能匹配', type: 'process', desc: 'SkillMatcher.match_and_execute() → 评分', color: '#f97316' },
                { node: 'skill 短路?', type: 'decision', desc: '高置信度命中 → 直接返回 skill 结果,跳过 LLM', color: '#dc2626' },
                { node: '身份注入', type: 'process', desc: 'SOUL.md + PROFILE.md + AGENTS.md + workspace/*', color: '#a855f7' },
                { node: '构建 Prompt', type: 'process', desc: '注入分类/记忆/计划/技能结果/身份 → messages[]', color: '#3b82f6' },
                { node: 'Main Loop: Think-Act', type: 'process', desc: 'LLM stream → tool_call → execute → result', color: '#8b5cf6' },
                { node: 'LLM 调用失败?', type: 'decision', desc: '529/超时/网络错误 → ErrorRecovery 重试', color: '#dc2626' },
                { node: 'CircuitBreaker', type: 'process', desc: '连续 5 次失败 → 跳闸 30s 后半开探测', color: '#b91c1c' },
                { node: 'Stage 5: 反思总结', type: 'process', desc: '同步 ReflectionEngine.reflect() → 渲染到前端', color: '#06b6d4' },

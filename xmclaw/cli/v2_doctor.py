"""``xmclaw v2 doctor`` — diagnose a v2 setup without running anything.

When ``xmclaw v2 serve`` fails or ``xmclaw v2 chat`` can't connect, the
...
def check_pairing_token(path: Path) -> CheckResult:
    ...
    if not path.exists():
        return CheckResult(
            ...
            detail=f"not yet created (will be created on `xmclaw v2 serve`)",
    ...
    if not content:
        return CheckResult(
            ...
            advisory="delete the file and re-run `xmclaw v2 serve` to regenerate",
        )
...
def check_daemon_health(host: str, port: int) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            name="daemon", ok=True,
            detail=f"not running at {host}:{port} (run `xmclaw v2 serve`)",
        )

...
def _default_system_prompt() -> str:
    ...
    return (
        ...
        "  规则：装完后告诉用户具体路径——B-173 起 SkillsWatcher 每\n"
        "  ~10s 重扫一次三个根目录，**新装的 SKILL.md 不需要重启 daemon\n"
        "  即可被注册**（已注册的同 id+version 重扫是 idempotent no-op）。\n"
        "  注：编辑既有 SKILL.md（同 id+version，body 改了）目前还需要\n"
        "  重启或 bump version；纯 Python skill.py 因为 importlib 缓存\n"
        "  也得重启。这两条限制后续会修。Phase 3 起 SkillProposer\n"
        "  会自己往规范路径产 candidate，evidence-gated promote 后才入库。\n"

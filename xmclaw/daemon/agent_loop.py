...
def _default_system_prompt() -> str:
    ...
    return (
        ...
        "Skill installation (Epic #24 Phase 5 — 单一路径接受两种格式):\n"
        "  规范路径就一个：``~/.xmclaw/skills_user/<skill_id>/``。\n"
        "  目录里二选一即可：\n"
        "    - ``skill.py`` —— Python ``Skill`` 子类（带代码逻辑）\n"
        "    - ``SKILL.md`` —— Markdown 步骤说明（你按步骤执行）\n"
        "  可选 ``manifest.json`` 标 permissions / version。\n"
        "  装新技能 **必须** 落进这里（或下面的 extra root），\n"
        "  否则 XMclaw 找不到。\n"
        "    - **正确**：``mkdir -p ~/.xmclaw/skills_user/<skill_id> && \n"
        "      cp <repo>/SKILL.md ~/.xmclaw/skills_user/<skill_id>/``。\n"
        "    - **正确**：``npx skills add`` 默认装到 ``~/.agents/skills/``，\n"
        "      用户可以在 ``daemon/config.json`` 里加 \n"
        "      ``\"evolution\": {\"skill_paths\": {\"extra\": [\"~/.agents/skills\"]}}`` \n"
        "      打开 opt-in 扫描；之后就跟规范路径一样进 SkillRegistry。\n"

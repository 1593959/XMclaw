    ...
    *,
    ...
    async def _lifespan(_app: FastAPI) -> AsyncIterator[None]:
        ...
        try:
            ...
            _intent_engine = getattr(_app.state, "intent_engine", None)
            # IntentEngine holds its store as ``_store`` (not ``store``) — the
            # previous ``.store.close()`` raised AttributeError on every
            # shutdown ("'IntentEngine' object has no attribute 'store'").
            _intent_store = getattr(_intent_engine, "_store", None)

...
def _default_system_prompt() -> str:
    ...
    return (
        ...
        "  - think: record PURE internal reasoning (planning, "
        "self-analysis, second-guessing, weighing tradeoffs) that the "
        "user should NOT see. Thoughts are logged for audit but never "
        "rendered in the chat bubble.\n"
        "    Use ``think`` for: '用户说X但其实可能是Y...让我先验证下...' / "
        "'这条路有点冒险,如果A不行就回退到B...' — the kind of inner "
        "monologue you'd cringe to read in a polished assistant reply.\n"
        "    DO NOT use ``think`` for short user-facing progress updates "
        "between tool calls. Those are NOT reasoning — they keep the "
        "user oriented while you're running a long chain. Always emit "
        "a 1-sentence plain-text update before / between substantial "
        "tool calls (file_read of a large file, bash that may take "
        "seconds, parallel_subagents fanout, multi-step refactor). "
        ...
        "    The shape that's WRONG (silent for 5+ minutes of tool "
        "calls, then 1 final reply): empty chat → user thinks you "
        "crashed. The shape that's RIGHT: 1 short plain-text update "
        "per ~2-3 tool calls, then tool calls, then summary at the "
        "end.\n"
        "    Rule of thumb: if it answers 'what am I doing right now / "
        "what did I just learn' → plain text. If it answers 'how do "
        "I feel about this / should I try a different approach' → "
        "``think``.\n"
        ...
        "    Keep the user informed without noise: each checkpoint "
        "ties a plan item to the latest tool result. Hop counts "

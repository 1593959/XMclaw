...
def _default_system_prompt() -> str:
    ...
    return (
        ...
        "  When a tool result contains something noteworthy (error, "
        "unexpected output, or a finding that changes your plan), start "
        "your next response with **1 short sentence** summarizing the "
        "key takeaway BEFORE the next tool call. If the result is "
        "straightforward or self-explanatory, emit the next tool call "
        "directly without narration — the user can read the tool card.\n"
        "  Format when you DO narrate: short, plain prose. No bullet "
        "lists, no headers, no emoji-spam. One sentence at most.\n"
        "  Examples of WORTH narrating:\n"
        "    ✅ 'list_dir 返回了 7 个 .md 文件,接下来读 LEARNING.md.'\n"
        "    ✅ 'daemon.log 有 6 MB, 先采样最后 50 行找错误.'\n"
        "  Examples of SKIP narrating:\n"
        "    ✅ file_read 成功返回了预期内容 → 直接调用下一个工具.\n"
        "    ✅ web_search 返回了 5 条标准结果 → 直接调用 web_fetch.\n"
        "  Skip narration on the VERY FIRST tool call of a turn (your "
        "initial '让我检查...' is enough) and on the FINAL synthesis hop.\n\n"
        "  CRITICAL: NEVER repeat information already stated in a "
        "previous hop's narration. If you already said '20 files, 3914 "
        "lines' in hop 1, do NOT say it again in hop 2, 3, etc. Each "
        "narration must contain ONLY new observations from the most "
        "recent tool result. Repetition wastes tokens and annoys the user.\n\n"
        "量化约束（减少 token 浪费）:\n"

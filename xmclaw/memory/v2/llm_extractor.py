...
【最重要：只记"稳定事实"，不记"当下指令"】
区分两类，**只输出前者**：
- ✅ 稳定事实/偏好/身份/长期目标/决定 —— 跨会话仍成立、值得长期记住的。
- ❌ **一次性命令 / 任务请求 / 临时上下文** —— 用户让你**现在**做某事、或针对当前
  回答的临时反应。这些是 TODO，不是记忆。例如：
    · "删除所有无法正常使用的技能" / "帮我改下配置" / "把这些清理掉" / "重启一下"
      → 是命令，**不要**存成"用户希望…/目标…"。
    · "不对，是另一个" / "这个理解错了" → 是对当前回答的临时纠正，**不要**存。
    · "现在/这次/先" 引导的、只对本回合有效的请求 → **不要**存。
  注意："以后都用中文""永远别删零调用技能"这种**立长期规矩**的才存（correction/
  preference）；"现在把X删了"这种**单次动作**不存。拿不准是不是长期 → **不输出**。

...
_CANVAS_CREATE_SPEC = ToolSpec(
    ...
    description=(
        ...
        "  • **演示文稿 / PPT / slide deck — build it as an html deck here "
        "(NOT a binary .pptx). The user watches it render live in the side "
        "preview and sees every slide appear/change as you go.**\n"
        ...
        "        window.xmclaw.submit(data)     — sends `data` (object, "
        "JSON-serialized) back as a structured form submission.\n"
        "      Wire these to onclick/onsubmit handlers to build UI the "
        "user can act on — their action arrives as the next user message.\n"
        "      **演示文稿 / PPT → 用 html 做一套自包含幻灯片（不要生成二进制 "
        ".pptx）：每页一个全宽 16:9 的 <section>，用 CSS scroll-snap 纵向翻页 "
        "(html,body{margin:0} .slide{width:100%;aspect-ratio:16/9;"
        "scroll-snap-align:start;display:flex;flex-direction:column;"
        "justify-content:center;padding:5%;box-sizing:border-box} "
        "容器 overflow-y:auto;scroll-snap-type:y mandatory)。先 canvas_create "
        "出第 1 页骨架拿到 artifact_id，之后每加/改一页都 canvas_update 推**整份 "
        "deck 的完整 HTML**（内容是全量替换，不是增量片段）——用户就能实时看到 "
        "一页页生成与修改。配色用内联 CSS，自包含、零外链。\n"
